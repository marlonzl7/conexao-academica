...
function irParaPagina(pagina) {
  paginaAtual = pagina;
  const termo =
    document.getElementById("pesquisa_instituicao")?.value.trim() ?? "";
  const dados = filtrarDados(termo);
  renderInstituicoes(dados);
  document
    .getElementById("lista_instituicoes")
...
function filtrarDados(termo) {
  ...
  return todasInstituicoes.filter((i) =>
...
async function executarPesquisa(termo) {
  try {
    const response = await fetch(
      `/administrador/search?termo=${encodeURIComponent(termo)}`,
    );
